...
class ExpenseController extends Controller
{
  ...
  //   	{
    ...
    public function store(Request $request)
    {
		...
		if(!empty($request->created_for) && Auth::user()->user_type==4)
		{
			$UserDeposit = UserDeposit::where('to_user', $request->created_for)->orderBy('id', 'desc')->first();
			$UserDeposit = UserDeposit::find($UserDeposit->id);
			$UserDeposit->expense_id = $id;
